alter table parents add column city text;

drop function if exists create_family_with_parent(text, text, time, text, text, text);
...
revoke all on function create_family_with_parent(text, text, text, time, text, text, text)
  from public, anon;
grant execute on function create_family_with_parent(text, text, text, time, text, text, text)
  to authenticated;
